...
public class ProductionRelationController extends BaseController {
    ...
    public ResponseEntity<JSONObject> listPage(@RequestBody SearchParams searchParams) {
        ...
        if (code == 100) {
            ...
            Map<Object, Object> objectObjectHashMap = data.stream()
                .collect(Collectors.toMap(x -> (x.get("id")), x -> (x.get("name"))));
            Optional.ofNullable(rows).ifPresent(x -> rows.forEach(row -> {
                Object o = objectObjectHashMap.get(row.getTenantId());
