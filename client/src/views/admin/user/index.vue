  ...
  methods: {
    ...
    async pageInit() {
      ...
      try {
        ...
        // 数据权限: 开发者sys_user
        this.pushDataPermissions(this.search.params, this.$route.meta.routerId, this.tableId)
        let [listUserRespData, listPermissionRespData] = await Promise.all([
          listUserPage(this.search),
          listResourcePermission(this.$route.meta.routerId)
        ])
        if (listUserRespData.code == 100 && listPermissionRespData.code == 100) {
          this.tableDataTotal = listUserRespData.data.total
          this.tableData = listUserRespData.data.rows ? listUserRespData.data.rows : []
          ...
          this.showMessage(listPermissionRespData.code != 100 ? listPermissionRespData : listUserRespData)
        }
        this.resetLoad()
      } catch (error) {
        this.outputError(error)
