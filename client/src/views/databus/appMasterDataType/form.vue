            ...
            >
              <el-row>
                ...
                <el-form-item
                  prop="masterDataType.id"
                  label="主数据字段"
                  :rules="[{ types: Array,required: true, message: '主数据字段不能为空', trigger: 'blur' | 'change' }]"
                  class="dc-el-form-item_SelectInput"
                >
                  <el-select
                    v-model="editFormData.masterDataTypeField"
                    :style="{ width: '100%' }"
                    placeholder="请选择主数据字段"
                    multiple="true"
                    clearable
                    value-key="id"
                    filterable
                  ...
                  >
                    <el-option
                      v-for="(item, index) in masterDataTypeFieldOptions"
                      :key="index"
...
/** 根据用户界面配置import组件 结束 */
import { listMasterDataTypeAll,listMicroVersionAll } from '@/api/databus/masterDataType.js'
/** 根据用户界面配置import组件 结束 */
import { getAppMasterDataTypeById, saveAppMasterDataType } from '@/api/databus/appMasterDataType'
import { getMasterDataTypeById } from '@/api/databus/masterDataType'
...
export default {
  ...
  methods: {
    ...
    doSave() {
      ...
      var masterDataTypeField = this.editFormData.masterDataTypeField;
      if (masterDataTypeField.length==0){
        this.showMessage({
          type: 'warning',
          msg: '请选择应用字段'
        })
      ...
      this.editFormData.masterDataType.standardFieldList=[]
      for (let i = 0; i < masterDataTypeField.length; i++) {
        this.editFormData.masterDataType.standardFieldList.push({id:masterDataTypeField[i]})
      }
      this.setLoad()
        ...
        getAppMasterDataTypeById(id)
          .then((responseData) => {
            ...
            if (responseData.code == 100) {
              ...
              var standardFieldList = responseData.data.masterDataType.standardFieldList;
              var masterDataTypeField =[]
              for (let i = 0; i < standardFieldList.length; i++) {
                masterDataTypeField.push(standardFieldList[i].id)
              }
              form.masterDataTypeField = masterDataTypeField
            } else {
              this.showMessage(responseData)
            }
            this.resetLoad()
    ...
    getFilesById() {
      this.editFormData.masterDataTypeField = [];
      var masterDataType = this.editFormData.masterDataType;
      for (let i = 0; i < this.masterDataTypeOptions.length; i++) {
        if (this.masterDataTypeOptions[i].id==masterDataType.id){
          this.masterDataTypeFieldOptions = this.masterDataTypeOptions[i].standardFieldList
        }
      }
      // getMasterDataTypeById(this.editFormData.masterDataType.id)
      //   .then((responseData) => {
      //     let form = {}
      //     if (responseData.code == 100) {
      //       this.masterDataTypeFieldOptions = responseData.data.standardFieldList
      //     } else {
      //       this.showMessage(responseData)
      //     }
      //   })
    ...
    listMasterDataTypeOptions() {
      ...
      listMasterDataTypeAll(search_List).then((responseData) => {
        if (responseData.code == 100) {
          ...
          for (let i = 0; i < this.masterDataTypeOptions.length; i++) {
            if (this.editFormData.masterDataType.id==this.masterDataTypeOptions[i].id){
              this.masterDataTypeFieldOptions = this.masterDataTypeOptions[i].standardFieldList
            }
          }
          console.log(this.editFormData.masterDataTypeField)
        } else {
          this.showMessage(responseData)
        }
      })
