        ...
        >
          <el-row>
            ...
            <el-form-item
              prop="rule"
              label="URL规则"
              :rules="[{ required: true, message: 'URL规则不能为空', trigger: 'blur' }]"
              class="dc-el-form-item_SingleInput"
            >
              <el-input
                v-model="editFormData.rule"
                :maxLength="128"
                placeholder="请输入连接地址规则"
                clearable
                class="dc-el-input_SingleInput"
              ></el-input>
            </el-form-item>
...
/** 根据用户界面配置import组件 结束 */
import { getSysDbTypeById, saveSysDbType } from '@/api/dbtype/sysDbType'
...
export default {
  ...
  methods: {
    ...
    doSave() {
      ...
      saveSysDbType(this.editFormData)
        .then((responseData) => {
