      ...
      <el-row style="height: 100%">
        <el-tabs v-model="tabName" type="border-card" tab-position="top" class="dc-el-tabs_ElTabs">
                ...
                >
                    ...
                    <template v-slot:content="{ row }">
                      <el-form-item label-width="0">
                        <div v-if="row.childColumn && row.childColumn.length > 0">
                          ...
                          关联条件：
                          <el-input v-model="row.associatedFilter" clearable style="width: 400px"></el-input>
                        </div>
                        <el-checkbox-group
                  ...
                  >
                      ...
                      >
                        ...
                        >
                          <el-option
                            ...
                            :label="item.name"
                            :value="item.name"
                            v-on:click.native="onChangeCustomType(row, item, null)"
                          ></el-option>
                        </el-select>
                        <!-- 选择custom类型 -->
                        <el-cascader 
                          v-else-if="row.javaType.value == 'Custom'"
                          v-model="row.customType.id"
                          :options="customType_DcTableColumn_List" 
                          :props="{ label: 'name', value: 'id', children: 'children', emitPath: false }"
                          separator="/"
                          filterable 
                          clearable
                          v-on:change="onChangeCustomType(row, null, $event)"></el-cascader>
                        <!-- 选择远程调用 -->
                        <el-cascader 
                          v-else-if="row.javaType.value == 'Remote'"
                          v-model="row.customType.id"
                          :options="project_DcTableColumn_List" 
                          :props="{ label: 'name', value: 'id', children: 'children', emitPath: false }"
                          separator="/"
                          filterable 
                          clearable
                    ...
                    <template v-slot="{ row, rowIndex }">
                      ...
                      >
                        <el-cascader 
                          v-if="row.javaType.value == 'Custom'"
                          v-model="row.customType.id"
                          :options="customType_DcTableColumn_List" 
                          :props="{ label: 'name', value: 'id', children: 'children', emitPath: false }"
                          separator="/"
                        ...
                        <el-cascader 
                          ...
                          clearable
                          v-on:change="onChangeCustomType(row, null, $event)"></el-cascader>
                        <span v-else-if="row.javaType.value == 'This'" class="my-input-sc">
          ...
          <el-tab-pane label="约束条件" :name="4" :disabled="action != 'view'" class="dc-el-tab-pane_ElTabPane">
            ...
            >
              <div v-for="(table, index) in editFormData.dcTableList" :key="index">
                <el-divider content-position="left" class="dc-el-divider_ElDivider">
...
import { listProjectTableColumn } from '@/api/project/project'
import { listTableColumn } from '@/api/dc/dcTable'
...
export default {
  ...
  data() {
    return {
      ...
      tableColumnIndex: 1, // 主表子表tab切换
      scheme_DcTable_List: [], // 代码方案
      customType_DcTableColumn_List: [], // 自定义类型(方案-业务表-业务表字段)
      project_DcTableColumn_List: [], // 远程调用(项目-方案-业务表-业务表字段)
      // 对话框属性变量
      dialogProps: {
        visible: false,
        title: '后端方案'
  ...
  methods: {
    ...
    doSave() {
      this.setLoad()
      // 更新代码
      if(this.editFormData.templateJson) {
        const dbModel = constructDbModel(this.editFormData)
        const json = JSON.parse(this.editFormData.templateJson)
        json.pages.forEach(page => {
          let newDbModel = dbModel
          // 子表代码文件处理
          if (page.hasOwnProperty('tableName')) {
            let childTable = dbModel.childTables.find((item) => {
          ...
          page.code = prettify(getExtensionName(page.fileName), code)
        })
        this.editFormData.templateJson = JSON.stringify(json)
      }
      ...
      if (!validatenull(this.editFormData.dcTableList)) {
        for (let i = 0; i < this.editFormData.dcTableList.length; i++) {
          ...
          this.editFormData.dcTableList[i].columnList.forEach((column) => {
            ...
            if (column.javaType.value == 'Custom' || column.javaType.value == 'This' || column.javaType.value == 'Remote') {
              // custom | thisObj
    ...
    async onTableChange(index, newValue, isMainTable) {
      ...
      if (validatenull(newValue)) {
        ...
        try {
          ...
          await listTableColumn(search, this.editFormData.project.id).then((responseData) => {
            if (responseData.code == 100) {
    ...
    onJavaTypeChange(row, javaType) {
      this.$set(row, 'currentSchemeTable', '') // 清空本方案
      row.customType.id = null // 自定义类型
      row.childFields = [] // 选中的关联表字段
      this.$set(row, 'childFieldKeys', []) // 选中的关联表字段Key
      row.childColumn = [] // 关联表字段
      row.associatedField = '' // 关联表对应的关联字段
      row.associatedFilter = '' // 关联表-->关联条件
      if (javaType && (javaType.value == 'This' || javaType.value == 'Custom' || javaType.value == 'Remote')) {
        this.$refs.dcTable[0].setRowExpand(row, true)
        if (
        ...
        this.$refs.dcTable[0].setRowExpand(row, false)
      }
    },
    ...
    setClassName({ row, rowIndex }) {
      ...
      if (row.javaType && (row.javaType.value == 'This' || row.javaType.value == 'Custom' || row.javaType.value == 'Remote')) {
        return '' // 可展开
      } else {
        return 'hide-expand' // 不可展开
      }
    },
    onChangeCustomType(row, val, customId) {
      if (!validatenull(row.javaType) && row.javaType.value == 'Custom') {
        for(let i = 0; i < this.customType_DcTableColumn_List.length; i++) {
          const scheme = this.customType_DcTableColumn_List[i]
          val = scheme.children.find((item) => {
            return item.id == customId //取出customType_DcTableColumn_List里的id相等的那条数据所有的信息
          })
          if (!validatenull(val)) {
            break;
          }
        }
      } else if (!validatenull(row.javaType) && row.javaType.value == 'Remote') {
        projectFlag: for(let i = 0; i < this.project_DcTableColumn_List.length; i++) {
          const project = this.project_DcTableColumn_List[i]
          schemeFlag: for (let j = 0; j < project.children.length; j++) {
            const scheme = project.children[j]
            val = scheme.children.find((item) => {
              return item.id == customId //取出project_DcTableColumn_List里的id相等的那条数据所有的信息
            }) 
            if (!validatenull(val)) {
              break projectFlag;
            }
      ...
      row.associatedField = '' // 关联表对应的关联字段
      row.associatedFilter = '' // 关联表-->关联条件
      if (!validatenull(val) && !validatenull(val.columnList)) {
    ...
    getTableList() {
      ...
      this.tableName_List = []
      listSchemaTableAll(st_search, this.editFormData.project.id).then((responseData) => {
    ...
    getCustomList() {
      ...
      this.customType_DcTableColumn_List = [] // 不包含项目的自定义类型
      this.project_DcTableColumn_List = [] // 远程调用自定义类型
      listProjectTableColumn(customType_search).then((responseData) => {
        const listAll = responseData.data
        if (validatenull(listAll)) return false
        this.project_DcTableColumn_List = listAll.filter(item => {
          return item.id != this.editFormData.project.id
        })
        const projectEntity = listAll.find((item) => {
          return item.id == this.editFormData.project.id
        })
        this.customType_DcTableColumn_List = projectEntity.children
        ...
        if (this.action == 'edit' || this.action == 'view' || this.action == 'copy') {
          this.editFormData.dcTableList.forEach((table) => {
            table.columnList.forEach((column) => {
              ...
              if (column.javaType.value == 'Custom' && !validatenull(this.customType_DcTableColumn_List)) {
                let custom = null
                for(let i = 0; i < this.customType_DcTableColumn_List.length; i++) {
                  const scheme = this.customType_DcTableColumn_List[i]
                  custom = scheme.children.find((item) => {
              ...
              } else if (column.javaType.value == 'Remote' && !validatenull(this.project_DcTableColumn_List)) { // 远程调用字段
                let remote = null
                projectFlag: for(let i = 0; i < this.project_DcTableColumn_List.length; i++) {
                  const project = this.project_DcTableColumn_List[i]
                  for (let j = 0; j < project.children.length; j++) {
                    const scheme = project.children[j]
                    remote = scheme.children.find((item) => {
                      return item.id == column.customType.id //取出project_DcTableColumn_List里的id相等的那条数据所有的信息
                    }) 
                    if (!validatenull(remote)) {
                      break projectFlag;
                    }
                  }
                }
                if (!validatenull(remote)) {
                  ...
                  this.$set(column, 'childFieldKeys', childFieldKeys) // 关联表字段
                  this.$set(column, 'childColumn', remote.columnList) // 关联表字段
                }
              }
            })
          })
    ...
    onRefreshColumn() {
      ...
      if (!validatenull(currentTable.name)) {
        ...
        listTableColumn(search, this.editFormData.project.id).then((responseData) => {
          if (responseData.code == 100) {
            const newColumns = []
