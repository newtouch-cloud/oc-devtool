      ...
      <el-main class="dc-el-main_ElMain">
        <el-row>
          ...
          <el-row gutter="0" type="flex" justify="end" align="top" class="dc-el-row_ElRow">
            <el-tooltip disabled class="dc-el-tooltip_Button" v-if="tableData.length > 0">
...
import { checkDataSource } from '@/api/project/project'
...
import { downBack, downBackScheme } from '@/api/dc/dcCode'
...
export default {
  ...
  methods: {
    ...
    onCreate() {
      ...
      //校验项目是否填写信息完整
      this.checkDataSource(this.parentTable.id).then((resp) => {
        if (resp.code == 100) {
    ...
    // 初始化自定义类型选择框选项变量
    initOptions(This) {},
    checkDataSource(projectId) {
      this.setLoad()
      return checkDataSource(projectId).then((responseData) => {
        this.resetLoad()
        return responseData
      })
    },
