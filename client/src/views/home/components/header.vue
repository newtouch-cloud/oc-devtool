...
<script>
  ...
  import { clearLocalData, removeLocalToken } from '@/utils/auth'
  export default {
    name: "NavHeader",
    components: { tabSite},
    props: {
    ...
    methods: {
      ...
      doLogOut() {
        this.$confirm('确定要退出系统吗?', '提示', {
          ...
          .then(responseData => {
            //removeLocalToken()
            sessionStorage.removeItem('dev_currentUser')
            sessionStorage.removeItem('dev_routers')
            sessionStorage.removeItem('dev_token')
            sessionStorage.removeItem('dev_personalTheme')
            sessionStorage.removeItem('dev_dataPermisions')
